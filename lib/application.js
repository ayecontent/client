...
"use strict";
var Application, SocketConnector, Sync, exec, fs, path, util;
...
Application = (function() {
  function Application(args) {
    ...
    this.eventHubConnector = new SocketConnector({
      logger: this.logger,
      config: this.config
    });
    this.sync = new Sync({
  ...
  Application.prototype.replaceBasePath = function(source, dest) {
    ...
    return fs.readFile(path.join(this.config.get('basepath'), "" + source + ".template"), 'utf8', (function(_this) {
      return function(err, data) {
        var result;
        result = data.replace(/%CLIENT_PATH%/g, _this.config.get('basepath')).replace(/%KEY_NAME%/g, _this.config.get('git:keyName'));
        return fs.writeFile(path.join(_this.config.get('basepath'), "" + dest), result, 'utf8');
      };
    })(this));
  };
