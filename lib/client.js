...
var rsyncQueue = async.queue(function (objectName, callback) {
  Executor.execute(new command.FullSyncCommand(clientOptions, function (result) {
    ...
    var buf = JSON.stringify({"command": "fullSync", "result": result});
    ...
    acknowledgeRequest.on('error', function (err) {
      logger.error(err);
    });
    acknowledgeRequest.write(buf);
...
//      logger.info('rsync operation from home folder started');
//      async.whilst(
//        function () {
  ...
  poolingRequest.on('response', function (res) {
    ...
    res.on('data', function (data) {  //message from server is received
      ...
      switch (message.command.toString()) {
        case 'getClientId':
          logger.debug('sending response to the server');
          var buf = JSON.stringify({"command": "getClientId", "result": clientOptions.clientId});
          var acknowledgeRequest = http.request({
            host: clientOptions.server,
            port: clientOptions.port,
            path: '/send',
